            ...
            <div class="detail-content" v-if="currentDetail">
                ...
                <div class="detail-section">
                    ...
                    <div class="ai-analysis-result">
                        <el-descriptions :column="2" border> 
                            <el-descriptions-item label="主要情绪" 
                            :type="getAiEmotionTagType(aiData.primaryEmotion)">{{ aiData.primaryEmotion || '无'}}
                            </el-descriptions-item>
                            <el-descriptions-item label="情绪强度">
                                <el-progress :percentage="aiData.emotionScore" :color="getEmotionScoreColor(aiData.emotionScore)" stroke-width="8"/>
                            </el-descriptions-item>
                            <el-descriptions-item label="风险等级" >
                                <el-tag :type="getRiskLevelTagType(aiData.riskLevel)">{{aiData.riskLevel || '无'}} </el-tag>
                            </el-descriptions-item>\
                ...
                <div class="detail-section">
                    ...
                    <el-descriptions :column="2" border> 
                        ...
                        <el-descriptions-item label="更新时间">{{ currentDetail.updatedAt || '无' }}</el-descriptions-item>
                    </el-descriptions>
                </div> 
            </div>
            <template #footer>
...
import { getEmotionPage, deleteEmotional} from '../api/admin'
...
const handleSearch = async (formDate) => {
    // 1. 打印接收的搜索参数（排查拼写错误）
    console.log('接收的搜索参数：', formDate);
    const params = {
        ...pagination,
        ...formDate
    }
    // 2. 打印最终传给接口的参数
    console.log('传给接口的参数：', params);
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确定删除改条记录吗？', '删除确认',{
